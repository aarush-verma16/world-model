...
def test_composite_hud_pastes_strip() -> None:
    from models.crafter_layout import HUD_H, HUD_W, HUD_TOP, composite_hud

    world = torch.zeros(2, 3, 64, 64)
    hud = torch.ones(2, 3, HUD_H, HUD_W)
    out = composite_hud(world, hud)
    assert out.shape == (2, 3, 64, 64)
    assert torch.allclose(out[:, :, HUD_TOP : HUD_TOP + HUD_H, :HUD_W], hud)
    assert torch.allclose(out[:, :, :HUD_TOP, :], torch.zeros(2, 3, HUD_TOP, 64))
