...
Open [`index.html`](index.html) in a browser for the interactive hub (filterable findings, hoverable 50k curves, figure galleries). The Markdown files under [`findings/`](findings/) are the cite-able writeups.
...
| 50k KL peak then decline | Results figure; “floor ≠ setpoint” |
| seq 64 thrash | Compute / hardware appendix |
...
5. [MSE ghosts vs crop losses](findings/05-mse-ghosts.md) — residue is not a missing head.
6. [Paper vs reference KL scales](findings/06-kl-scale-mismatch.md) — Table 4 vs size-S YAML; 50k shape.
7. [Diagnostics that lie](findings/07-lying-diagnostics.md) — resume plots, log-continue, embed-vs-`[h,z]`.
8. [Workstation recipe](findings/08-workstation-recipe.md) — seq 64, replay size, VRAM, Jupyter kernels.
...
- **M3 reset** (`configs/m3_dreamer_s.yaml`): size-S, one live decoder, paper four-term loss. **50k steps, 2026-08-24.** `recon_l1` 0.326 → 0.0095; `kl_rep_raw` peaked 6.14 at step 16650, finished 3.49; reward correlation **r = 0.91**. Continuation toward 300k is in progress and should be folded into finding 05/06 when it finishes — do not treat 50k as the final sprite-quality number.
