...
- **M8 actor-critic fix** (finding 17): all three M7 runs above shared six actor-critic bugs, so none of them tested XL or `train_ratio`. Fixed in `src/training/returns.py` (`imagined_targets`), `src/training/imagine.py` and `src/training/ac_step.py`. Next runs are `configs/m8_s_acfix.yaml` (size-S on M6's 1M world model, 200k, ~1 h — run this first) then `configs/m8_xl_acfix.yaml` (XL 1M). Both smoke clean; neither has been trained yet.
